...
async def _dailyMensa(botti, botData):
    while(True):
        ...
        if nextPing.weekday() < 4:
            msg = modules.bottiHelper._createDummyMessage(botti.get_guild(botData.ETIT_KIT_ServerID).get_member(botti.user.id), discord.utils.get(botti.get_all_channels(), id = ids.channelIDs.mensa_ChannelID), "")
